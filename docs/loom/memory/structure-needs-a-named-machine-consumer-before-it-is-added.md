...
- **Consumer-checked structure caught real defects.** `critic-found`
  ...
  aggregation rule (evidence never gates) and demonstrably stopped the
  🟡-accumulation loop; `origin:` tags are consumed by a validator that
  verifies them against committed content.
